...
def show_chart(titles, images):
    plt.figure(figsize=(15, 10))
    for i in range(len(images)):
        plt.subplot(2, 4, i+1)
        plt.hist(images[i].ravel(), bins=256, range=[0, 256], alpha=0.7, color='gray')
        plt.title(f"Histogram {titles[i]}", fontsize=14)
...
for img_file in image_files:
    ...
    # Show histogram of each image
    show_chart(titles, images)
